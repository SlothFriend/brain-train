...
export default class Trainer {
      ...
      brains.forEach(brain => {
        if (this.modelBrains.length < this.#maxModelBrains) {
          ...
          for (let i = 0; i < this.modelBrains.length; i++) {
            console.log('Model brain cost:', this.modelBrains[i].cost)
            console.log('New brain cost:', brain.cost)
  ...
  cull (tieBreakWithCost: boolean = false): number[] {
    ...
    const toReplace = indexWithScores.slice(indexWithScores.length - Math.round(indexWithScores.length / 3))
    toReplace.forEach(({ index }) => {
      // Replace the brain with a copy of one of the best scoring brains
      this.#trainingBrains[index] = _sample(this.modelBrains).copy().prune()
    })
